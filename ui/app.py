...
/* Preserve Material Symbols Rounded font for expander toggle arrows */
[data-testid="stMainBlockContainer"] [data-testid="stIconMaterial"] {
...
/* ── Sidebar ── */
[data-testid="stSidebar"] { background: #1b2e2b !important; }
[data-testid="stSidebarContent"] { padding-top: 0.75rem !important; }
section[data-testid="stSidebar"] > div:first-child { padding-top: 0 !important; }
[data-testid="stSidebar"] * { color: #b8ccc9 !important; }
[data-testid="stSidebar"] hr { border-color: #263d39 !important; }

/* ── Status banner ── */
.banner { border-radius:12px; padding:20px 28px; margin-bottom:20px; display:flex; align-items:center; gap:20px; }
.banner-paid     { background:linear-gradient(135deg,#ecfdf5,#d1fae5); border:1px solid #6ee7b7; }
.banner-rejected { background:linear-gradient(135deg,#fff1f0,#ffe4e2); border:1px solid #fca5a5; }
.banner-error    { background:linear-gradient(135deg,#fffbeb,#fef3c7); border:1px solid #fcd34d; }
...
.pipe-stage:last-child  { border-radius:0 10px 10px 0; border-right:1px solid #e5e1d8; }
.pipe-stage.ps-pass { background:#f0fdf8; border-color:#6ee7b7; }
.pipe-stage.ps-fail { background:#fff1f0; border-color:#fca5a5; }
.pipe-stage.ps-warn { background:#fffbeb; border-color:#fcd34d; }
.pipe-stage.ps-skip { background:#faf9f7; border-color:#e5e1d8; }
.pipe-num  { font-size:.65rem; font-weight:700; text-transform:uppercase; letter-spacing:.08em; color:#a8a29e; }
.pipe-name { font-size:.9rem; font-weight:700; margin:2px 0; color:#1c1917; }
.pipe-detail { font-size:.78rem; color:#78716c; }
...
.flag-error   { background:#fff1f0; border-left:3px solid #ef4444; }
.flag-warning { background:#fffbeb; border-left:3px solid #f59e0b; }
.flag-info    { background:#f0fdf8; border-left:3px solid #0f766e; }
.flag-code { font-weight:700; font-size:.75rem; white-space:nowrap; padding-top:1px; }
.flag-error   .flag-code { color:#dc2626; }
.flag-warning .flag-code { color:#d97706; }
.flag-info    .flag-code { color:#0f766e; }
...
/* ── Reasoning card ── */
.reason-card { background:#fff; border:1px solid #e5e1d8; border-radius:10px;
               padding:16px 20px; margin:12px 0; }
.reason-card h4 { margin:0 0 8px; font-size:.85rem; text-transform:uppercase;
                  letter-spacing:.06em; color:#78716c; }
.reason-card p { margin:0; font-size:.9rem; line-height:1.6; color:#1c1917; }

/* ── Line items table ── */
.li-table { width:100%; border-collapse:collapse; font-size:.875rem; }
.li-table th { background:#faf9f7; padding:8px 10px; text-align:left;
               font-size:.75rem; text-transform:uppercase; letter-spacing:.05em; color:#78716c; }
.li-table td { padding:8px 10px; border-bottom:1px solid #f5f3ef; color:#1c1917; }
.li-table tr:last-child td { border-bottom:none; }
.li-table .amount { text-align:right; font-weight:600; }

/* ── Metric cards ── */
.biz-metric { background:#fff; border:1px solid #e5e1d8; border-radius:10px;
              padding:16px 20px; text-align:center; }
.biz-metric-val { font-size:1.8rem; font-weight:700; margin:4px 0; color:#1c1917; }
.biz-metric-label { font-size:.78rem; color:#78716c; text-transform:uppercase; letter-spacing:.05em; }

/* ── Sidebar stat ── */
.sb-stat { background:#243b38; border-radius:10px; padding:14px 16px; margin-bottom:8px;
           border:1px solid #2f4f4b; }
.sb-stat-val { font-size:1.4rem; font-weight:800; color:#e8f5f3; letter-spacing:-.02em; }
.sb-stat-label { font-size:.68rem; color:#6b9e99; text-transform:uppercase; letter-spacing:.07em; margin-top:2px; }

/* ── Confidence bar ── */
.conf-bar-wrap { background:#e5e1d8; border-radius:4px; height:8px; margin-top:4px; }
.conf-bar { background:#0f766e; border-radius:4px; height:8px; }

/* ── Invoice detail card ── */
.detail-card { background:#fff; border:1px solid #e5e1d8; border-radius:12px; padding:20px 24px; }
...
.detail-item-label { font-size:.72rem; color:#a8a29e; text-transform:uppercase;
                     letter-spacing:.05em; margin-bottom:2px; }
.detail-item-val { font-size:.95rem; font-weight:600; color:#1c1917; }
.detail-item-val.missing { color:#dc2626; font-style:italic; font-weight:400; }

/* ── Verdict card ── */
.verdict-approved { background:linear-gradient(135deg,#ecfdf5,#d1fae5);
                    border:1px solid #6ee7b7; border-radius:12px; padding:20px 24px; margin-bottom:16px; }
.verdict-rejected { background:linear-gradient(135deg,#fff1f0,#fee2e2);
                    border:1px solid #fca5a5; border-radius:12px; padding:20px 24px; margin-bottom:16px; }
.verdict-label { font-size:1.3rem; font-weight:700; margin:0 0 4px; color:#1c1917; }
.verdict-sub   { font-size:.85rem; opacity:.75; margin:0; }

/* ── Receipt card ── */
.receipt { background:#fff; border:1px solid #e5e1d8; border-radius:12px;
           padding:24px; max-width:460px; }
.receipt-header { display:flex; justify-content:space-between; align-items:flex-start;
                  margin-bottom:20px; padding-bottom:16px; border-bottom:1px solid #f5f3ef; }
.receipt-title  { font-size:1rem; font-weight:700; color:#1c1917; margin:0; }
.receipt-badge-paid { background:#d1fae5; color:#065f46; font-size:.72rem;
...
.receipt-row { display:flex; justify-content:space-between; margin-bottom:10px;
               font-size:.875rem; }
.receipt-row-label { color:#78716c; }
.receipt-row-val   { font-weight:600; color:#1c1917; }
.receipt-amount { font-size:1.6rem; font-weight:700; color:#1c1917;
                  text-align:right; margin-top:16px; padding-top:16px;
                  border-top:2px solid #f5f3ef; }
.receipt-txn { font-size:.72rem; color:#a8a29e; text-align:right; margin-top:4px;
               font-family:monospace; }

/* ── Empty state ── */
.empty-state { text-align:center; padding:48px 24px; }
.empty-state-title { font-size:1.1rem; font-weight:700; color:#1c1917; margin:16px 0 8px; }
.empty-state-sub { font-size:.875rem; color:#78716c; margin:0; }
.how-it-works { display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin:32px 0; }
.how-step { background:#fff; border:1px solid #e5e1d8; border-radius:12px;
            padding:20px 16px; text-align:center; }
.how-step-num { width:32px; height:32px; border-radius:50%; display:inline-flex;
                align-items:center; justify-content:center; font-weight:700;
                font-size:.8rem; margin-bottom:10px; }
.how-step-num-1 { background:#f0fdf8; color:#0f766e; }
.how-step-num-2 { background:#f0fdf4; color:#16a34a; }
.how-step-num-3 { background:#fffbeb; color:#b45309; }
.how-step-num-4 { background:#fff7ed; color:#c2410c; }
.how-step-name { font-size:.85rem; font-weight:700; color:#1c1917; margin-bottom:4px; }
.how-step-desc { font-size:.75rem; color:#78716c; line-height:1.4; }
...
.page-heading h1 {
    ...
    color: #1c1917; letter-spacing: -.03em; line-height: 1.15;
}
.page-heading h1 span {
    background: linear-gradient(135deg, #0f766e, #0d9488);
    -webkit-background-clip: text; -webkit-text-fill-color: transparent;
}
.page-heading p { margin: 0; font-size: .925rem; color: #78716c; line-height: 1.6; }
...
.custom-upload-zone-title { font-weight: 600; color: #1c1917; font-size: .9rem; margin-bottom: 4px; }
.custom-upload-zone-sub { font-size: .78rem; color: #78716c; }
...
with st.sidebar:
    ...
    <div style="padding:4px 0 16px">
        <div style="font-size:1.05rem;font-weight:800;color:#e8f5f3;letter-spacing:-.02em">
            🏭 Acme Corp
        </div>
        <div style="font-size:.72rem;color:#2dd4bf;font-weight:600;letter-spacing:.06em;
    ...
    <div class="sb-stat">
        <div class="sb-stat-val" style="color:#6ee7b7">${stats['paid_amount']:,.0f}</div>
